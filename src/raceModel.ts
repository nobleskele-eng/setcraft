...
export type RaceReference = {
  ...
  dataClass?: "observed" | "derived" | "coach";
...
export function genericCumulativeModel(event: string, total: number): number[] {
  ...
  return normalizeFactors(checkpoints, strategies[Math.min(1, strategies.length - 1)].paceFactors, total);
}

export function confidenceScore(event: string, course: Course, cumulative: number[], total: number) {
  let score = 45;
  if (course === "LCM") score += 15;
  if (cumulative.length === defaultCheckpoints(event).length) score += 30;
  if (Math.abs((cumulative.at(-1) || 0) - total) < 0.11) score += 10;
