...
const CATEGORY_META: Record<string, { dot: string; selected: string; palette: string; icon: React.ElementType }> = {
  Structure: { dot: "bg-slate-500", selected: "bg-slate-100 text-slate-900", palette: "bg-slate-500 hover:bg-slate-600", icon: Layers3 },
  "Warm-up": { dot: "bg-cyan-500", selected: "bg-cyan-50 text-cyan-800", palette: "bg-cyan-500 hover:bg-cyan-600", icon: Waves },
  Skills: { dot: "bg-emerald-500", selected: "bg-emerald-50 text-emerald-800", palette: "bg-emerald-500 hover:bg-emerald-600", icon: Target },
  "Kick & Pull": { dot: "bg-amber-500", selected: "bg-amber-50 text-amber-800", palette: "bg-amber-500 hover:bg-amber-600", icon: Droplets },
  Aerobic: { dot: "bg-blue-500", selected: "bg-blue-50 text-blue-800", palette: "bg-blue-500 hover:bg-blue-600", icon: Activity },
  Threshold: { dot: "bg-orange-500", selected: "bg-orange-50 text-orange-800", palette: "bg-orange-500 hover:bg-orange-600", icon: Gauge },
  Speed: { dot: "bg-red-500", selected: "bg-red-50 text-red-800", palette: "bg-red-500 hover:bg-red-600", icon: Zap },
  "Race pace": { dot: "bg-fuchsia-500", selected: "bg-fuchsia-50 text-fuchsia-800", palette: "bg-fuchsia-500 hover:bg-fuchsia-600", icon: Flag },
  Lactate: { dot: "bg-purple-600", selected: "bg-purple-50 text-purple-800", palette: "bg-purple-600 hover:bg-purple-700", icon: Activity },
  USRPT: { dot: "bg-indigo-600", selected: "bg-indigo-50 text-indigo-800", palette: "bg-indigo-600 hover:bg-indigo-700", icon: TimerReset },
  Recovery: { dot: "bg-teal-500", selected: "bg-teal-50 text-teal-800", palette: "bg-teal-500 hover:bg-teal-600", icon: RefreshCw },
  Control: { dot: "bg-amber-600", selected: "bg-amber-50 text-amber-900", palette: "bg-amber-600 hover:bg-amber-700", icon: Blocks },
  "Favorites": { dot: "bg-yellow-500", selected: "bg-yellow-50 text-yellow-900", palette: "bg-yellow-500 hover:bg-yellow-600", icon: Star },
  "My Blocks": { dot: "bg-rose-500", selected: "bg-rose-50 text-rose-800", palette: "bg-rose-500 hover:bg-rose-600", icon: Sparkles },
  "Backpack": { dot: "bg-violet-600", selected: "bg-violet-50 text-violet-900", palette: "bg-violet-600 hover:bg-violet-700", icon: ClipboardList },
};

const NODE_STYLE: Record<string, { shell: string; header: string; badge: string; border: string }> = {
  section: { shell: "bg-sky-50/70 border-sky-300", header: "bg-sky-500", badge: "bg-sky-100 text-sky-800 border-sky-200", border: "border-sky-300" },
  repeat: { shell: "bg-amber-50/70 border-amber-300", header: "bg-amber-500", badge: "bg-amber-100 text-amber-900 border-amber-200", border: "border-amber-300" },
  condition: { shell: "bg-violet-50/70 border-violet-300", header: "bg-violet-500", badge: "bg-violet-100 text-violet-900 border-violet-200", border: "border-violet-300" },
  progress: { shell: "bg-orange-50/70 border-orange-300", header: "bg-orange-500", badge: "bg-orange-100 text-orange-900 border-orange-200", border: "border-orange-300" },
  "time-cap": { shell: "bg-indigo-50/70 border-indigo-300", header: "bg-indigo-500", badge: "bg-indigo-100 text-indigo-900 border-indigo-200", border: "border-indigo-300" },
  lane: { shell: "bg-pink-50/70 border-pink-300", header: "bg-pink-500", badge: "bg-pink-100 text-pink-900 border-pink-200", border: "border-pink-300" },
  note: { shell: "bg-yellow-50 border-yellow-300", header: "bg-yellow-400", badge: "bg-yellow-100 text-yellow-900 border-yellow-200", border: "border-yellow-300" },
  "warm-up": { shell: "bg-cyan-500 border-cyan-600", header: "bg-cyan-600", badge: "bg-cyan-100 text-cyan-900 border-cyan-200", border: "border-cyan-600" },
  drill: { shell: "bg-emerald-500 border-emerald-600", header: "bg-emerald-600", badge: "bg-emerald-100 text-emerald-900 border-emerald-200", border: "border-emerald-600" },
  underwater: { shell: "bg-teal-500 border-teal-600", header: "bg-teal-600", badge: "bg-teal-100 text-teal-900 border-teal-200", border: "border-teal-600" },
  kick: { shell: "bg-amber-500 border-amber-600", header: "bg-amber-600", badge: "bg-amber-100 text-amber-900 border-amber-200", border: "border-amber-600" },
  pull: { shell: "bg-orange-500 border-orange-600", header: "bg-orange-600", badge: "bg-orange-100 text-orange-900 border-orange-200", border: "border-orange-600" },
  aerobic: { shell: "bg-blue-500 border-blue-600", header: "bg-blue-600", badge: "bg-blue-100 text-blue-900 border-blue-200", border: "border-blue-600" },
  threshold: { shell: "bg-orange-600 border-orange-700", header: "bg-orange-700", badge: "bg-orange-100 text-orange-900 border-orange-200", border: "border-orange-700" },
  sprint: { shell: "bg-red-500 border-red-600", header: "bg-red-600", badge: "bg-red-100 text-red-900 border-red-200", border: "border-red-600" },
  "race-pace": { shell: "bg-fuchsia-500 border-fuchsia-600", header: "bg-fuchsia-600", badge: "bg-fuchsia-100 text-fuchsia-900 border-fuchsia-200", border: "border-fuchsia-600" },
  lactate: { shell: "bg-purple-600 border-purple-700", header: "bg-purple-700", badge: "bg-purple-100 text-purple-900 border-purple-200", border: "border-purple-700" },
  USRPT: { shell: "bg-indigo-600 border-indigo-700", header: "bg-indigo-700", badge: "bg-indigo-100 text-indigo-900 border-indigo-200", border: "border-indigo-700" },
  "test-set": { shell: "bg-slate-600 border-slate-700", header: "bg-slate-700", badge: "bg-slate-100 text-slate-900 border-slate-200", border: "border-slate-700" },
  recovery: { shell: "bg-teal-500 border-teal-600", header: "bg-teal-600", badge: "bg-teal-100 text-teal-900 border-teal-200", border: "border-teal-600" },
};
...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return <label className="mb-1.5 block text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{children}</label>;
}
...
function DropZone({ parentId, index, active, label, onDropPayload }: { parentId: string | null; index: number; active: boolean; label?: string; onDropPayload: (parentId: string | null, index: number, event: React.DragEvent<HTMLDivElement>) => void }) {
  ...
  return (
    ...
    >
      <div className={`mx-1 flex h-full w-full items-center justify-center rounded-xl border-2 border-dashed transition-all ${hovered ? "border-cyan-500 bg-cyan-50 shadow-[0_0_0_5px_rgba(6,182,212,0.14)]" : active ? "border-cyan-300/80 bg-cyan-50/45" : "border-transparent group-hover:border-slate-300"}`}>
        {hovered && <span className="rounded-full bg-cyan-600 px-3 py-1 text-[10px] font-bold text-white shadow-sm">Snap {label || "block"} here</span>}
      </div>
    </div>
  );
}
...
function NodeCard({ node, selectedId, draggingId, dragActive, dragLabel, collapsedIds, poolUnit, onSelect, onDelete, onDuplicate, onPatch, onToggleCollapse, onDragStart, onDragEnd, onDropPayload, onQuickAdd }: NodeCardProps) {
  ...
  if (node.kind === "set") {
    ...
    return (
      <div onClick={(event) => { event.stopPropagation(); onSelect(node.id); }} className={`scratch-stack-block relative rounded-[14px] border-2 text-white shadow-sm transition ${style.shell} ${selected ? "ring-4 ring-slate-900/70 ring-offset-2" : "hover:-translate-y-0.5 hover:shadow-md"} ${draggingId === node.id ? "opacity-35" : "opacity-100"}`}>
        <div className="flex min-h-11 items-center gap-2 px-3 py-2">
          ...
          <span className="rounded-full bg-black/15 px-2 py-0.5 text-[11px] font-extrabold uppercase tracking-wide">{TYPE_LABEL[node.blockType]}</span>
          ...
          <div className="flex flex-wrap items-center gap-1.5 border-t border-white/20 px-3 py-2.5 text-[11px] font-semibold">
            ...
            <StopDrag>
              <select aria-label="Stroke" value={node.stroke} onChange={(event) => patchSet({ stroke: event.target.value })} className="scratch-input min-w-20">
                {STROKES.map((stroke) => <option key={stroke} className="text-slate-900">{stroke}</option>)}
              </select>
            </StopDrag>
            <span>on</span>
            <StopDrag>
  ...
  if (node.kind === "note") {
    return (
      <div onClick={(event) => { event.stopPropagation(); onSelect(node.id); }} className={`relative rounded-xl border-2 p-3 text-slate-800 shadow-sm ${style.shell} ${selected ? "ring-4 ring-slate-900/70 ring-offset-2" : ""}`}>
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[11px] leading-relaxed text-slate-600">{node.text}</p>
          </div>
          <button type="button" disabled={node.locked} onClick={(event) => { event.stopPropagation(); onDelete(node.id); }} className="rounded p-1 text-slate-500 hover:bg-yellow-100 disabled:opacity-40"><Trash2 className="h-4 w-4" /></button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div onClick={(event) => { event.stopPropagation(); onSelect(node.id); }} className={`scratch-c-block relative rounded-[18px] border-2 p-2.5 shadow-sm transition ${style.shell} ${selected ? "ring-4 ring-slate-900/70 ring-offset-2" : "hover:shadow-md"} ${draggingId === node.id ? "opacity-35" : "opacity-100"}`}>
      <div className={`flex items-center gap-2 rounded-xl px-3 py-2 text-white ${style.header}`}>
        ...
        <span className="rounded-full bg-black/15 px-2 py-0.5 text-[11px] font-extrabold uppercase tracking-wide">{kindLabel}</span>
        {node.kind === "repeat" && (
          <StopDrag>
      ...
      {!collapsed && (
        <>
          {node.kind === "condition" && (
            <div className="mt-2 flex flex-wrap items-center gap-1.5 px-2 text-[10px] font-semibold text-violet-900">
              <span className="rounded-full bg-white px-2 py-1">IF {node.metric} {node.comparator} {node.threshold}</span>
              <span className="rounded-full bg-violet-100 px-2 py-1">THEN {node.action}</span>
              <span className="rounded-full bg-white px-2 py-1">ELSE {node.elseAction}</span>
            </div>
          )}
          {node.kind === "lane" && (
            <div className="mt-2 flex flex-wrap gap-1.5 px-2">
              {node.lanes.map((lane) => <span key={lane.id} className="rounded-full border border-pink-200 bg-white px-2 py-1 text-[11px] font-extrabold text-pink-800">{lane.name}: {lane.targetPace} · {lane.intervalAdjustmentSeconds >= 0 ? "+" : ""}{lane.intervalAdjustmentSeconds}s</span>)}
            </div>
          )}

          <div
            className={`mt-2 min-h-24 rounded-[14px] border-2 border-dashed bg-white/80 p-2 transition ${style.border} ${dragActive ? "ring-2 ring-cyan-200/70" : ""}`}
            onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = "move"; }}
            onDrop={(event) => {
          ...
          >
            ...
            {children.length === 0 && <div className="pointer-events-none flex min-h-12 items-center justify-center text-[11px] font-semibold text-slate-400">Drop blocks inside this control block</div>}
            <div className="mt-1 flex flex-wrap items-center justify-center gap-1.5 border-t border-slate-100 pt-2">
              <button type="button" onClick={(event) => { event.stopPropagation(); onQuickAdd(node.id, "cruise"); }} className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-extrabold text-slate-600 hover:bg-slate-50">+ swim</button>
              <button type="button" onClick={(event) => { event.stopPropagation(); onQuickAdd(node.id, "easy-swim"); }} className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-extrabold text-slate-600 hover:bg-slate-50">+ recovery</button>
              <button type="button" onClick={(event) => { event.stopPropagation(); onQuickAdd(node.id, "repeat"); }} className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-extrabold text-slate-600 hover:bg-slate-50">+ repeat</button>
            </div>
          </div>
        </>
      )}
    ...
    <div className="w-full" id="setcraft-swim-studio">
      <section className="overflow-hidden bg-white">
        <div className="flex flex-wrap items-center gap-2 bg-gradient-to-r from-slate-950 via-indigo-950 to-slate-950 px-4 py-2 text-white">
          ...
          <button type="button" onClick={() => setTemplateOpen((open) => !open)} className="flex items-center gap-1 rounded-lg px-2.5 py-2 text-[11px] font-bold hover:bg-white/10"><ArrowDownToLine className="h-4 w-4" /> File <ChevronDown className="h-3 w-3" /></button>
          <button type="button" onClick={undo} disabled={history.length === 0} className="rounded-lg p-2 hover:bg-white/10 disabled:opacity-35" title="Undo"><Undo2 className="h-4 w-4" /></button>
          <button type="button" onClick={redo} disabled={future.length === 0} className="rounded-lg p-2 hover:bg-white/10 disabled:opacity-35" title="Redo"><Redo2 className="h-4 w-4" /></button>
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <button type="button" onClick={() => { setStudioPage("build"); setQuickWriteOpen(true); }} className="premium-button flex items-center gap-1.5 rounded-xl bg-white/10 px-3 py-2 text-xs font-extrabold text-white hover:bg-white/20"><PenLine className="h-4 w-4" /> Quick write</button>
            <button type="button" onClick={() => { setRightTab("analysis"); setStudioPage("review"); }} className="premium-button flex items-center gap-1.5 rounded-xl bg-white/10 px-3 py-2 text-xs font-extrabold text-white ring-1 ring-white/10 hover:bg-white/20"><ShieldCheck className="h-4 w-4" /> Validate</button>
            <button type="button" onClick={saveTemplate} className="premium-button flex items-center gap-1.5 rounded-xl bg-blue-600 px-3 py-2 text-xs font-extrabold text-white shadow-sm shadow-blue-950/30 hover:bg-blue-500"><Save className="h-4 w-4" /> {saveNotice ? "Saved" : "Save project"}</button>
            <button type="button" onClick={exportPdf} className="premium-button flex items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-xs font-extrabold text-slate-900 shadow-sm hover:bg-slate-100"><FileDown className="h-4 w-4" /> {pdfNotice ? "PDF ready" : "Export PDF"}</button>
        ...
        {templateOpen && (
          <div className="absolute z-50 ml-4 mt-2 w-80 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <div><p className="text-xs font-bold text-slate-900">Workout projects</p><p className="text-[10px] text-slate-400">Saved by season folder on this device</p></div>
              <button type="button" onClick={() => setTemplateOpen(false)} className="rounded p-1 text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button>
            </div>
            <div className="border-b border-slate-100 p-2"><button type="button" onClick={startNewProject} className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-3 py-2.5 text-xs font-bold text-white hover:bg-indigo-700"><Plus className="h-4 w-4" /> New blank project</button></div><div className="max-h-80 overflow-y-auto p-2">
              {savedTemplates.length === 0 ? <p className="px-3 py-8 text-center text-xs text-slate-400">No saved workouts yet.</p> : savedTemplates.map((template) => (
                <div key={template.id} className="group flex items-center gap-2 rounded-xl p-2 hover:bg-slate-50">
                  <button type="button" onClick={() => loadTemplate(template)} className="min-w-0 flex-1 text-left"><p className="truncate text-xs font-bold text-slate-800">{template.name}</p><p className="mt-1 text-[10px] text-slate-400">{template.folder || "Inbox"} · {template.poolLength}{template.poolUnit} · {template.targetMinutes} min</p></button>
                  <button type="button" onClick={() => deleteTemplate(template.id)} className="rounded p-1.5 text-slate-300 opacity-0 hover:bg-rose-50 hover:text-rose-600 group-hover:opacity-100"><Trash2 className="h-4 w-4" /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        <nav className="studio-workflow border-b border-slate-200 bg-white px-4 py-2" aria-label="Swim Studio workflow">
          <div className="flex w-full flex-wrap items-center gap-1.5">
            {([
              ...
              return (
                <button key={item.id} type="button" onClick={() => setStudioPage(item.id)} title={item.helper} className={`group flex items-center gap-2 rounded-lg border px-3 py-1.5 text-left transition ${active ? "border-indigo-200 bg-indigo-50 text-indigo-950 shadow-sm" : "border-transparent text-slate-500 hover:bg-slate-100"}`}>
                  <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md transition ${active ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-500 group-hover:bg-slate-200"}`}><Icon className="h-3.5 w-3.5" /></span>
                  <span className="whitespace-nowrap text-[13px] font-bold">{item.label}</span>
                  {active && <span className="hidden max-w-[220px] truncate text-[11px] font-semibold text-indigo-400/90 xl:inline">· {item.helper}</span>}
                </button>
              );
            })}
          </div>
        ...
        {studioPage === "project" && (
          <div className="studio-stage-page min-h-[960px] bg-gradient-to-b from-slate-50 via-white to-indigo-50/40 px-6 py-8 lg:px-10 lg:py-10">
            <div className="mx-auto max-w-[1500px] space-y-7">
              <header className="flex flex-wrap items-end justify-between gap-5 rounded-[28px] border border-slate-200 bg-white p-7 shadow-sm">
                <div>
                  <div className="flex items-center gap-2 text-indigo-600"><FolderPlus className="h-5 w-5" /><span className="text-xs font-extrabold uppercase tracking-[0.18em]">Project setup</span></div>
                  <h2 className="mt-3 font-display text-3xl font-extrabold tracking-tight text-slate-950">Create the workout project before building the sets.</h2>
                  <p className="mt-2 max-w-3xl text-base leading-relaxed text-slate-600">Give the practice a clear identity, connect it to a season folder, and define the training context. Everything here travels with the saved project and appears in your Project Hub.</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={startNewProject} className="premium-button flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-extrabold text-slate-700 hover:border-indigo-300 hover:bg-indigo-50"><Plus className="h-4 w-4" /> New project</button>
                  <button type="button" onClick={saveTemplate} className="premium-button flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-extrabold text-white shadow-lg shadow-indigo-200 hover:bg-indigo-700"><Save className="h-4 w-4" /> Save project</button>
                </div>
              </header>

              <div className="grid gap-7 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,.65fr)]">
                <section className="rounded-[28px] border border-slate-200 bg-white p-7 shadow-sm">
                  <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-5"><div><h3 className="font-display text-xl font-extrabold text-slate-950">Project identity</h3><p className="mt-1 text-sm text-slate-500">The information coaches use to find and understand this practice later.</p></div><span className="rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-extrabold text-indigo-700">Auto-saved</span></div>
                  <div className="mt-6 space-y-5">
                    <label className="block"><span className="mb-2 block text-xs font-extrabold uppercase tracking-[0.13em] text-slate-500">Workout project name</span><input value={sessionName} onChange={(event) => setSessionName(event.target.value)} className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-4 font-display text-xl font-extrabold text-slate-950 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100" placeholder="e.g. Friday speed endurance" /></label>
                    <label className="block"><span className="mb-2 block text-xs font-extrabold uppercase tracking-[0.13em] text-slate-500">Training focus</span><textarea value={focus} onChange={(event) => setFocus(event.target.value)} className="studio-meta-input min-h-[110px] resize-y" placeholder="Describe the purpose, event focus and desired adaptation." /></label>
                ...
                <aside className="space-y-7">
                  <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm"><h3 className="font-display text-lg font-extrabold text-slate-950">Training context</h3><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><CompactField label="Pool length"><div className="grid grid-cols-2 gap-2"><select value={poolLength} onChange={(event) => setPoolLength(Number(event.target.value) as PoolLength)} className="studio-meta-input"><option value={25}>25</option><option value={50}>50</option></select><select value={poolUnit} onChange={(event) => setPoolUnit(event.target.value as PoolUnit)} className="studio-meta-input"><option value="m">Metres</option><option value="yd">Yards</option></select></div></CompactField><CompactField label="Booking time"><div className="relative"><input type="number" min={10} max={240} value={targetMinutes} onChange={(event) => setTargetMinutes(Math.max(10, Number(event.target.value) || 10))} className="studio-meta-input pr-14" /><span className="pointer-events-none absolute right-4 top-3.5 text-xs font-bold text-slate-400">min</span></div></CompactField></div></section>

                  <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><h3 className="font-display text-lg font-extrabold text-slate-950">Season folder</h3><p className="mt-1 text-sm text-slate-500">Keep practices grouped by phase, squad or meet cycle.</p></div><FolderPlus className="h-5 w-5 text-indigo-500" /></div><select value={projectFolder} onChange={(event) => setProjectFolder(event.target.value)} className="studio-meta-input mt-5">{projectFolders.map((folder) => <option key={folder}>{folder}</option>)}</select><div className="mt-3 flex gap-2"><input value={newFolderName} onChange={(event) => setNewFolderName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") addProjectFolder(); }} placeholder="Create another folder" className="studio-meta-input" /><button type="button" onClick={addProjectFolder} className="flex shrink-0 items-center gap-1.5 rounded-xl bg-slate-950 px-4 py-3 text-sm font-extrabold text-white hover:bg-indigo-700"><Plus className="h-4 w-4" /> Add</button></div><div className="mt-4 flex flex-wrap gap-2">{projectFolders.slice(0, 8).map((folder) => <button key={folder} type="button" onClick={() => setProjectFolder(folder)} className={`rounded-full border px-3 py-1.5 text-xs font-bold ${projectFolder === folder ? "border-indigo-300 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-slate-50 text-slate-500 hover:border-cyan-300 hover:bg-cyan-50"}`}>{folder}</button>)}</div></section>
                </aside>
              </div>

              <section className="grid gap-4 rounded-[28px] border border-slate-200 bg-slate-950 p-6 text-white shadow-xl md:grid-cols-4"><div><p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Current distance</p><p className="mt-2 text-2xl font-extrabold">{stats.totalDistance.toLocaleString()} {poolUnit}</p></div><div><p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Estimated duration</p><p className="mt-2 text-2xl font-extrabold">{stats.estimatedDuration} min</p></div><div><p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Lane groups</p><p className="mt-2 text-2xl font-extrabold">{laneAssignments.lanes.length}</p></div><button type="button" onClick={() => setStudioPage("build")} className="premium-button flex items-center justify-center gap-2 rounded-2xl bg-cyan-400 px-5 py-4 text-base font-extrabold text-slate-950 hover:bg-cyan-300">Continue to Build Sets <ChevronRight className="h-5 w-5" /></button></section>
            </div>
          </div>
        )}
        ...
        {studioPage === "build" && (
          <>
        <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-indigo-50/40 px-6 pt-4 pb-3.5"><div className="flex flex-wrap items-baseline gap-x-3.5 gap-y-1.5"><span className="text-[10px] font-extrabold uppercase leading-none tracking-[0.16em] text-indigo-500">Building</span><h2 className="font-display text-base font-extrabold leading-none text-slate-950">{sessionName}</h2><span className="text-xs font-medium leading-none text-slate-400">{phase} · {projectFolder} · {targetMinutes} min</span></div><button type="button" onClick={() => setStudioPage("project")} className="shrink-0 rounded-lg border border-slate-300 bg-white px-3.5 py-1.5 text-[13px] font-extrabold text-slate-700 transition hover:border-indigo-300 hover:bg-indigo-50">Edit project setup</button></div>
        ...
        <div className={`grid min-h-[1080px] grid-cols-1 ${paletteVisible && inspectorVisible ? "xl:grid-cols-[390px_minmax(720px,1fr)_430px]" : paletteVisible ? "xl:grid-cols-[390px_minmax(760px,1fr)]" : inspectorVisible ? "xl:grid-cols-[minmax(760px,1fr)_430px]" : "xl:grid-cols-1"}`}>
          {paletteVisible && (
          <aside className="grid min-h-[940px] grid-cols-[108px_minmax(0,1fr)] border-r border-slate-200 bg-white">
            <div className="border-r border-slate-200 bg-slate-50 py-2">
              {categories.map((category) => {
                const meta = CATEGORY_META[category] || CATEGORY_META.Structure;
                const Icon = meta.icon;
                const active = activeCategory === category && !searchQuery;
                return (
                  <button type="button" key={category} onClick={() => { setPaletteSearch(""); setActiveCategory(category); }} className={`flex w-full flex-col items-center gap-1 px-1 py-2 text-center text-[10px] font-extrabold leading-tight transition ${active ? meta.selected : "text-slate-500 hover:bg-white"}`}>
                  ...
                  return (
                    <div key={item.id} className="group relative">
                      <button type="button" draggable onDragStart={(event) => handlePaletteDragStart({ source: "palette", paletteId: item.id }, event)} onDragEnd={handleDragEnd} onClick={() => addFromPalette(item.id)} className={`palette-puzzle w-full cursor-grab rounded-xl px-3 py-2.5 pr-9 text-left text-white shadow-sm transition active:cursor-grabbing ${meta.palette}`}>
                        <div className="flex items-center gap-2"><GripVertical className="h-4 w-4 opacity-70" /><span className="text-sm font-extrabold">{item.label}</span><Plus className="ml-auto h-4 w-4" /></div>
          ...
          <main className={`min-w-0 bg-[#f7fbff] ${inspectorVisible ? "border-r border-slate-200" : ""}`}>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2.5 border-b border-slate-200 bg-white px-5 py-2.5">
              <div className="flex items-center gap-2"><CircleDot className="h-4 w-4 text-indigo-600" /><span className="text-sm font-bold text-slate-800">Scripts</span></div>
              <div className="ml-auto flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => setPaletteVisible((value) => !value)} className={`flex h-9 w-9 items-center justify-center rounded-lg border transition ${paletteVisible ? "border-indigo-200 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`} title={paletteVisible ? "Hide block library" : "Show block library"}><PanelLeftClose className="h-4 w-4" /></button>
                  <button type="button" onClick={() => setInspectorVisible((value) => !value)} className={`flex h-9 w-9 items-center justify-center rounded-lg border transition ${inspectorVisible ? "border-indigo-200 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`} title={inspectorVisible ? "Hide inspector" : "Show inspector"}><PanelRightClose className="h-4 w-4" /></button>
                </div>
                <span className="h-6 w-px bg-slate-200" />
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => setCanvasZoom((value) => Math.max(0.7, Number((value - 0.1).toFixed(1))))} className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50" title="Zoom out"><Minus className="h-4 w-4" /></button>
                  <button type="button" onClick={() => setCanvasZoom(1)} className="flex h-9 min-w-[3.5rem] items-center justify-center rounded-lg border border-slate-200 bg-white px-2 text-xs font-extrabold text-slate-500 transition hover:bg-slate-50" title="Reset zoom to 100%">{Math.round(canvasZoom * 100)}%</button>
                  <button type="button" onClick={() => setCanvasZoom((value) => Math.min(1.4, Number((value + 0.1).toFixed(1))))} className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50" title="Zoom in"><Plus className="h-4 w-4" /></button>
                  <button type="button" onClick={() => setCanvasZoom(nodes.length > 16 ? 0.8 : nodes.length > 9 ? 0.9 : 1)} className="flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-extrabold text-slate-600 transition hover:bg-slate-50" title="Fit the workout stack to the workspace"><Maximize2 className="h-4 w-4" /> Fit</button>
                </div>
                <span className="h-6 w-px bg-slate-200" />
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => setCollapsedIds(new Set(containerIds))} className="flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-extrabold text-slate-600 transition hover:bg-slate-50" title="Collapse every section and control block"><Layers3 className="h-4 w-4" /> Collapse</button>
                  <button type="button" onClick={() => setCollapsedIds(new Set())} className="flex h-9 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-extrabold text-slate-600 transition hover:bg-slate-50" title="Expand every section and control block">Expand</button>
                </div>
                <span className="h-6 w-px bg-slate-200" />
                <div className="flex items-center gap-1.5">
                  <button type="button" onClick={() => { commitNodes(starterWorkout()); setSelectedId(null); }} className="flex h-9 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-extrabold text-slate-600 transition hover:bg-slate-50" title="Load the starter workout">Starter</button>
                  <button type="button" onClick={() => { commitNodes([]); setSelectedId(null); }} className="flex h-9 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-extrabold text-rose-600 transition hover:border-rose-200 hover:bg-rose-50" title="Clear the canvas">Clear</button>
                </div>
              </div>
            </div>

            <div
              className={`studio-grid min-h-[1030px] overflow-auto p-5 transition ${dragActive ? "bg-cyan-50/60" : ""}`}
            ...
            >
              <div style={{ transform: `scale(${canvasZoom})`, transformOrigin: "top left", width: `${100 / canvasZoom}%` }}>
                ...
                {nodes.length === 0 && (
                  <div className="mx-auto mt-24 max-w-md rounded-2xl border-2 border-dashed border-slate-300 bg-white/80 px-8 py-12 text-center shadow-sm">
                    <Blocks className="mx-auto h-9 w-9 text-indigo-300" />
                    <h3 className="mt-4 font-display text-base font-bold text-slate-800">Build your first set</h3>
                    <p className="mt-2 text-xs leading-relaxed text-slate-500">Choose a category, drag a block into the scripts area, then place blocks inside repeats, progressions, lane branches and conditions.</p>
                    <button type="button" onClick={(event) => { event.stopPropagation(); commitNodes(starterWorkout()); }} className="mt-4 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-indigo-700">Load starter workout</button>
                  </div>
                )}
              </div>
            </div>
          ...
          <aside className="min-w-0 bg-white">
            ...
            <div className="max-h-[1010px] overflow-y-auto p-5">
              {rightTab === "inspector" && (
                !selectedNode ? (
                  <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-12 text-center"><CircleDot className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-xs font-bold text-slate-600">Select a block</p><p className="mt-1 text-[10px] leading-relaxed text-slate-400">Every block has editable parameters. Basic values can also be changed directly on the canvas.</p></div>
                ) : (
                  <div className="space-y-4">
                    <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
                      <div className="min-w-0"><span className={`inline-flex rounded-md border px-2 py-0.5 text-[11px] font-extrabold uppercase tracking-wider ${NODE_STYLE[styleKey(selectedNode)].badge}`}>{selectedNode.kind === "set" ? TYPE_LABEL[selectedNode.blockType] : selectedNode.kind}</span><p className="mt-2 truncate text-sm font-bold text-slate-900">{nodeLabel(selectedNode)}</p></div>
                      <div className="flex gap-1"><button type="button" onClick={() => patchSelected((node) => ({ ...node, locked: !node.locked } as StudioNode))} className={`rounded-lg border p-2 ${selectedNode.locked ? "border-amber-200 bg-amber-50 text-amber-700" : "border-slate-200 text-slate-500 hover:bg-slate-50"}`} title={selectedNode.locked ? "Unlock" : "Lock"}>{selectedNode.locked ? <Lock className="h-4 w-4" /> : <LockOpen className="h-4 w-4" />}</button><button type="button" onClick={() => duplicateNode(selectedNode.id)} className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-50"><Copy className="h-4 w-4" /></button><button type="button" disabled={selectedNode.locked} onClick={() => deleteNode(selectedNode.id)} className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-40"><Trash2 className="h-4 w-4" /></button></div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2"><button type="button" onClick={saveSelectedAsCustom} className="flex items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2.5 text-xs font-bold text-rose-700 hover:bg-rose-100"><Sparkles className="h-4 w-4" /> My Blocks</button><button type="button" onClick={saveSelectedToBackpack} className="flex items-center justify-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2.5 text-xs font-bold text-violet-700 hover:bg-violet-100"><ClipboardList className="h-4 w-4" /> Backpack</button></div>
                  </div>
                )
              )}
              ...
              {rightTab === "preview" && (
                <div className="space-y-4">
                  <div><h2 className="text-sm font-bold text-slate-950">Pool-deck preview</h2><p className="mt-1 text-[10px] text-slate-400">A clean text view generated from the same block graph.</p></div>
                  <div className="rounded-2xl bg-slate-950 p-4 text-white shadow-inner"><div className="flex items-center justify-between border-b border-slate-700 pb-3"><div><p className="font-display text-sm font-bold">{sessionName}</p><p className="mt-1 text-[9px] text-slate-400">{focus} · {stats.totalDistance.toLocaleString()}{poolUnit} · {stats.estimatedDuration} min</p></div><Waves className="h-5 w-5 text-cyan-400" /></div><pre className="mt-4 whitespace-pre-wrap font-mono text-[10px] leading-relaxed text-slate-200">{previewText || "Build a set to see the deck preview."}</pre></div>
                  <button type="button" onClick={() => navigator.clipboard?.writeText(previewText)} className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50"><Copy className="h-4 w-4" /> Copy workout text</button>
                  {onSaveWorkoutToCalendar && <button type="button" onClick={pushToCalendar} className="w-full rounded-xl bg-indigo-600 px-3 py-2.5 text-xs font-bold text-white hover:bg-indigo-700">Add to training week</button>}
                </div>
              )}
            </div>
          </aside>
        ...
        {studioPage === "review" && (
          <div className="bg-slate-50 p-4 md:p-6">
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
              <section className="professional-card overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-slate-950 px-5 py-5 text-white">
                  <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-cyan-400 text-slate-950"><MonitorUp className="h-5 w-5" /></span>
                  <div><h2 className="font-display text-lg font-extrabold">Pool-deck preview</h2><p className="mt-1 text-xs text-slate-400">The same structured workout that will be used for PDF export.</p></div>
                  <button type="button" onClick={exportPdf} className="premium-button ml-auto flex items-center gap-2 rounded-xl bg-cyan-400 px-4 py-3 text-sm font-extrabold text-slate-950 hover:bg-cyan-300"><FileDown className="h-4 w-4" /> Export PDF</button>
                </div>
                <div className="p-5 md:p-7">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white shadow-inner">
                    <div className="border-b border-slate-200 bg-slate-50 px-5 py-4"><h3 className="font-display text-xl font-extrabold text-slate-950">{sessionName}</h3><p className="mt-1 text-sm text-slate-500">{focus} · {phase}</p></div>
                    <pre className="max-h-[820px] overflow-y-auto whitespace-pre-wrap p-5 font-mono text-xs leading-7 text-slate-700">{previewText || "Build a workout to see the preview."}</pre>
                  </div>
                </div>
              </section>
              ...
              <aside className="space-y-5">
                ...
                <section className="professional-card rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-4 grid gap-2">
                    <button type="button" onClick={saveTemplate} className="premium-button flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-extrabold text-white hover:bg-indigo-700"><Save className="h-4 w-4" /> Save project</button>
                    <button type="button" onClick={exportPdf} className="flex items-center justify-center gap-2 rounded-xl border border-cyan-300 bg-cyan-50 px-4 py-3 text-sm font-extrabold text-cyan-800 hover:bg-cyan-100"><FileDown className="h-4 w-4" /> Export one-page PDF</button>
                    <button type="button" onClick={exportJson} className="flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-extrabold text-slate-700 hover:bg-slate-50"><FileJson className="h-4 w-4" /> Export structured JSON</button>
                    <button type="button" onClick={() => navigator.clipboard?.writeText(previewText)} className="flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-extrabold text-slate-700 hover:bg-slate-50"><Copy className="h-4 w-4" /> Copy workout text</button>
                  </div>
                  <div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500"><span className="font-extrabold text-slate-700">Auto-save:</span> {autoSaveState === "saving" ? "Saving current draft…" : "Current draft saved locally."}</div>
                </section>
              </aside>
            </div>
          </div>
      ...
      {dropNotice && (
        <div className="fixed bottom-6 left-1/2 z-[80] -translate-x-1/2 rounded-full bg-slate-950 px-5 py-3 text-xs font-bold text-white shadow-2xl">
          {dropNotice}
        </div>
      )}
      ...
      {shortcutsOpen && (
        <ModalFrame
          ...
          footer={<button type="button" onClick={() => setShortcutsOpen(false)} className="rounded-xl bg-slate-950 px-5 py-2.5 text-sm font-extrabold text-white hover:bg-indigo-700">Done</button>}
        >
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            ].map(([key, description]) => <div key={key} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><kbd className="rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 font-mono text-xs font-bold text-slate-800 shadow-sm">{key}</kbd><p className="mt-3 text-sm leading-6 text-slate-600">{description}</p></div>)}
          </div>
        </ModalFrame>
      )}
...
function LaneAssignmentPanel({ config, onChange, deckMeta, onDeckMetaChange, open, onToggleOpen, workoutOptions, mode = "all" }: {
  ...
  const metaInput = "w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-medium text-slate-800 outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100";
  const miniInput = "w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-50";
    ...
    <section className="professional-card overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center gap-3 px-5 py-4">
        <button type="button" onClick={mode === "all" ? onToggleOpen : undefined} className="flex items-center gap-3 text-left">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-100 text-cyan-700"><Users className="h-5 w-5" /></span>
          <span><span className="block text-base font-extrabold text-slate-950">{panelTitle}</span><span className="mt-1 block text-sm text-slate-500">{panelHelper}</span></span>
          {mode === "all" && (open ? <ChevronDown className="h-5 w-5 text-slate-400" /> : <ChevronRight className="h-5 w-5 text-slate-400" />)}
        </button>
        {(mode === "all" || mode === "lanes") && <div className="ml-auto flex flex-wrap items-center gap-2">
          <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-bold text-slate-700">
            <button type="button" role="switch" aria-checked={config.enabled} onClick={() => onChange((current) => ({ ...current, enabled: !current.enabled }))} className={`relative h-6 w-11 rounded-full transition ${config.enabled ? "bg-emerald-500" : "bg-slate-300"}`}>
              <span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${config.enabled ? "left-6" : "left-1"}`} />
            </button>
            Include lane table
          </label>
          <span className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-bold text-slate-600">{config.lanes.length} lane{config.lanes.length === 1 ? "" : "s"}</span>
          <button type="button" onClick={addLane} className="premium-button flex items-center gap-1.5 rounded-xl bg-cyan-600 px-4 py-2.5 text-sm font-extrabold text-white hover:bg-cyan-700"><Plus className="h-4 w-4" /> Add lane</button>
        </div>}
      </div>

      {open && (
        <div className="space-y-6 border-t border-slate-100 bg-slate-50/70 px-5 py-5">
          {(mode === "all" || mode === "deck") && (
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between"><div><h3 className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-600">Practice header</h3><p className="mt-1 text-xs text-slate-500">These fields appear at the top and bottom of the exported deck sheet.</p></div></div>
          ...
          <div className={`${config.enabled ? "opacity-100" : "opacity-60"}`}>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <div><h3 className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-600">Lane roster & lane-specific set plan</h3><p className="mt-1 text-xs text-slate-500">Assign a section or set to a lane and override pace, send-off, repetitions or distance.</p></div>
            ...
            <div className="grid gap-4 xl:grid-cols-2 2xl:grid-cols-3">
              {config.lanes.map((lane, laneIndex) => (
                <div key={lane.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="flex items-center gap-2">
                    <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-950 text-xs font-extrabold text-white">{laneIndex + 1}</span>
                    <input value={lane.label} onChange={(event) => updateLane(lane.id, { label: event.target.value })} className="min-w-0 flex-1 border-0 bg-transparent text-sm font-extrabold text-slate-950 outline-none" />
                    <button type="button" onClick={() => addSwimmer(lane.id)} className="rounded-lg bg-cyan-50 p-2 text-cyan-700 hover:bg-cyan-100" title="Add swimmer"><Users className="h-4 w-4" /></button>
                    <button type="button" onClick={() => removeLane(lane.id)} disabled={config.lanes.length <= 1} className="rounded-lg p-2 text-slate-300 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-30" title="Remove lane"><Trash2 className="h-4 w-4" /></button>
                  </div>
                  ...
                  <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50 p-3">
                    <div className="flex items-center justify-between"><p className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600">Swimmers</p><button type="button" onClick={() => addSwimmer(lane.id)} className="text-[10px] font-bold text-cyan-700">+ Add swimmer</button></div>
                    <div className="mt-2 space-y-2">
                      {lane.swimmers.map((swimmer) => (
                        <div key={swimmer.id} className="rounded-xl border border-slate-200 bg-white p-2.5">
                          <div className="flex gap-1.5"><input value={swimmer.name} onChange={(event) => updateSwimmer(lane.id, swimmer.id, { name: event.target.value })} placeholder="Swimmer name" className="min-w-0 flex-1 rounded-lg border border-slate-200 px-2.5 py-2 text-xs font-bold outline-none focus:border-cyan-400" /><button type="button" onClick={() => removeSwimmer(lane.id, swimmer.id)} className="rounded-lg p-2 text-slate-300 hover:bg-rose-50 hover:text-rose-500"><Trash2 className="h-4 w-4" /></button></div>
                          <input value={swimmer.assignment} onChange={(event) => updateSwimmer(lane.id, swimmer.id, { assignment: event.target.value })} placeholder="Personal assignment / pace / equipment" className="mt-2 w-full rounded-lg border border-slate-200 px-2.5 py-2 text-[11px] outline-none focus:border-cyan-400" />
                          <input value={swimmer.notes} onChange={(event) => updateSwimmer(lane.id, swimmer.id, { notes: event.target.value })} placeholder="Personal note or restriction" className="mt-2 w-full rounded-lg border border-slate-200 px-2.5 py-2 text-[11px] outline-none focus:border-cyan-400" />
                        </div>
                      ))}
                      {lane.swimmers.length === 0 && <button type="button" onClick={() => addSwimmer(lane.id)} className="w-full rounded-xl border border-dashed border-slate-300 px-3 py-4 text-xs font-bold text-slate-500 hover:border-cyan-400 hover:bg-cyan-50 hover:text-cyan-700">+ Add swimmer</button>}
                    </div>
                  </div>

                  <div className="mt-3 rounded-xl border border-pink-100 bg-pink-50/60 p-3">
                    <div className="flex items-center justify-between"><div><p className="text-[11px] font-extrabold uppercase tracking-wider text-pink-800">Set assignments</p><p className="mt-0.5 text-[10px] text-pink-700/70">Overrides print beside the matching section.</p></div><button type="button" onClick={() => addSetAssignment(lane.id)} className="rounded-lg bg-pink-600 px-2.5 py-1.5 text-[10px] font-bold text-white hover:bg-pink-700">+ Assign set</button></div>
                    <div className="mt-2 space-y-2">
                      {lane.setAssignments.map((assignment) => (
                        <div key={assignment.id} className="rounded-xl border border-pink-100 bg-white p-2.5">
                          <div className="flex gap-1.5">
                            <select value={assignment.nodeId} onChange={(event) => { const selected = workoutOptions.find((option) => option.id === event.target.value); updateSetAssignment(lane.id, assignment.id, { nodeId: event.target.value, nodeLabel: selected?.label.replace(/^—\s*/g, "") || "Set" }); }} className="min-w-0 flex-1 rounded-lg border border-slate-200 px-2 py-2 text-[11px] font-bold outline-none focus:border-pink-400">
                              {workoutOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
                            </select>
                            <button type="button" onClick={() => removeSetAssignment(lane.id, assignment.id)} className="rounded-lg p-2 text-slate-300 hover:bg-rose-50 hover:text-rose-500"><Trash2 className="h-4 w-4" /></button>
                          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-center gap-3">
              <div><h3 className="text-xs font-extrabold uppercase tracking-[0.14em] text-slate-600">Target goal-time tables</h3><p className="mt-1 text-xs text-slate-500">Optional tables printed at the bottom of the deck sheet.</p></div>
              <label className="ml-auto flex items-center gap-2 text-xs font-bold text-slate-700"><button type="button" role="switch" aria-checked={deckMeta.goalTimesEnabled} onClick={() => onDeckMetaChange((current) => ({ ...current, goalTimesEnabled: !current.goalTimesEnabled }))} className={`relative h-6 w-11 rounded-full transition ${deckMeta.goalTimesEnabled ? "bg-emerald-500" : "bg-slate-300"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${deckMeta.goalTimesEnabled ? "left-6" : "left-1"}`} /></button>Include in PDF</label>
              <button type="button" onClick={addGoalTable} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">+ Add table</button>
            </div>
...
function QuickSetComposer({ draft, onChange, onAdd, destination, poolUnit }: {
  ...
  const field = "rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100";
  const labelText = "mb-1 block text-[11px] font-bold uppercase tracking-wide text-slate-500";
  if (!open) {
    return (
      <div className="flex items-center gap-3 border-b border-slate-200 bg-white px-6 py-2">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-indigo-100 text-indigo-700"><Zap className="h-[18px] w-[18px]" /></span>
        <div className="min-w-0"><p className="text-[13px] font-extrabold leading-tight text-slate-900">Quick set composer</p><p className="truncate text-[11px] font-medium text-slate-400">Fast-add a set to {destination}</p></div>
        <button type="button" onClick={() => setOpen(true)} className="ml-auto flex shrink-0 items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700 transition hover:bg-indigo-100"><Plus className="h-4 w-4" /> Quick add <ChevronDown className="h-3.5 w-3.5" /></button>
      </div>
    );
  }
  return (
    <div className="flex flex-wrap items-end gap-x-4 gap-y-2.5 border-b border-slate-200 bg-white px-6 py-3">
      <button type="button" onClick={() => setOpen(false)} title="Collapse quick composer" className="mr-1 flex items-center gap-2.5 border-r border-slate-200 pr-4 text-left"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-100 text-indigo-700"><Zap className="h-[18px] w-[18px]" /></span><span><span className="flex items-center gap-1 text-[13px] font-extrabold leading-tight text-slate-900">Quick set composer <ChevronUp className="h-3.5 w-3.5 text-slate-400" /></span><span className="block text-[11px] font-medium text-slate-400">Adds to {destination}</span></span></button>
      ...
      <button type="button" onClick={onAdd} className="flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-indigo-700"><Plus className="h-4 w-4" /> Add set</button>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/55 p-4" role="dialog" aria-modal="true">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
          <div><h2 className="font-display text-lg font-bold text-slate-950">{title}</h2><p className="mt-1 text-xs leading-relaxed text-slate-500">{subtitle}</p></div>
          <button type="button" onClick={onClose} className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-50" aria-label="Close"><X className="h-4 w-4" /></button>
        </div>
        <div className="max-h-[65vh] overflow-y-auto px-6 py-5">{children}</div>
        <div className="flex items-center justify-end gap-2 border-t border-slate-100 bg-slate-50 px-6 py-4">{footer}</div>
      </div>
    </div>
  );
}
...
function QuickWriteModal({ value, destination, onChange, onClose, onAdd }: { value: string; destination: string; onChange: (value: string) => void; onClose: () => void; onAdd: () => void }) {
  return (
    <ModalFrame
      ...
      onClose={onClose}
      footer={<><button type="button" onClick={onClose} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50">Cancel</button><button type="button" onClick={onAdd} className="rounded-xl bg-indigo-600 px-5 py-2.5 text-xs font-bold text-white hover:bg-indigo-700">Add structured blocks</button></>}
    >
      <div className="space-y-4">
        <div className="rounded-xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-xs text-indigo-800"><span className="font-bold">Destination:</span> {destination}. Select an unlocked section or repeat before opening Quick Write to add directly inside it.</div>
        <textarea value={value} onChange={(event) => onChange(event.target.value)} rows={13} autoFocus spellCheck={false} className="w-full resize-y rounded-2xl border border-slate-200 bg-slate-950 p-4 font-mono text-sm leading-relaxed text-slate-100 outline-none focus:border-indigo-400" />
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Common notation</p><pre className="mt-2 whitespace-pre-wrap font-mono text-[11px] leading-relaxed text-slate-700"># Warm-up{"\n"}4x100 Choice @ 1:40 RPE 4 - build by 25{"\n"}8x50 Free @ 1:00 target :34 - paddles</pre></div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Nested repeat</p><pre className="mt-2 whitespace-pre-wrap font-mono text-[11px] leading-relaxed text-slate-700">Repeat 3x: Race circuit{"\n"}4x25 Fly @ :40 RPE 8{"\n"}1x100 Choice @ 2:00 - easy{"\n"}end</pre></div>
        </div>
        <p className="text-[10px] leading-relaxed text-slate-400">Recognized details include stroke, send-off, target time, rest seconds, RPE, equipment names, notes, section headings, repeat groups and coach notes. Anything unrecognized is safely added as a note instead of being discarded.</p>
      </div>
    </ModalFrame>
  );
}
...
function MakeBlockModal({ draft, onChange, onClose, onCreate }: { draft: MakeBlockDraft; onChange: React.Dispatch<React.SetStateAction<MakeBlockDraft>>; onClose: () => void; onCreate: () => void }) {
  ...
  return (
    <ModalFrame
      ...
      onClose={onClose}
      footer={<><button type="button" onClick={onClose} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50">Cancel</button><button type="button" onClick={onCreate} className="rounded-xl bg-rose-500 px-5 py-2.5 text-xs font-bold text-white hover:bg-rose-600">Create My Block</button></>}
    >
      <div className="space-y-5">
        <div>
          <FieldLabel>Block shape</FieldLabel>
          <div className="grid gap-2 md:grid-cols-2">{kinds.map((kind) => <button type="button" key={kind.value} onClick={() => patch({ kind: kind.value })} className={`rounded-xl border p-3 text-left transition ${draft.kind === kind.value ? "border-rose-300 bg-rose-50 ring-2 ring-rose-100" : "border-slate-200 bg-white hover:bg-slate-50"}`}><p className={`text-xs font-bold ${draft.kind === kind.value ? "text-rose-800" : "text-slate-800"}`}>{kind.label}</p><p className="mt-1 text-[10px] leading-relaxed text-slate-500">{kind.description}</p></button>)}</div>
        </div>
        ...
        {draft.kind === "section" && <div className="rounded-2xl border border-sky-100 bg-sky-50/60 p-4"><FieldLabel>Section purpose</FieldLabel><textarea value={draft.sectionPurpose} onChange={(event) => patch({ sectionPurpose: event.target.value })} rows={4} placeholder="e.g. Quality race-pace work. Drag any number or type of blocks inside." className={`${inputClass} resize-none`} /><p className="mt-2 text-[10px] text-sky-700">Custom sections are empty when placed and accept sets, repeats, conditions, notes and other sections without a block limit.</p></div>}
...
function CompactField({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="mt-3 block md:mt-0"><span className="mb-1 block text-[11px] font-extrabold uppercase tracking-wider text-slate-400">{label}</span>{children}</label>;
}
...
function TabButton({ active, onClick, icon: Icon, label, badge }: { active: boolean; onClick: () => void; icon: React.ElementType; label: string; badge?: number }) {
  return <button type="button" onClick={onClick} className={`relative flex items-center justify-center gap-1.5 rounded-lg px-2 py-2 text-[10px] font-bold transition ${active ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:bg-white/60"}`}><Icon className="h-4 w-4" />{label}{badge ? <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-500 px-1 text-[8px] text-white">{badge}</span> : null}</button>;
}

function Metric({ label, value, suffix }: { label: string; value: string; suffix: string }) {
  return <div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><p className="text-[11px] font-extrabold uppercase tracking-wider text-slate-400">{label}</p><p className="mt-1 font-display text-lg font-bold text-slate-950">{value} <span className="text-[10px] font-semibold text-slate-400">{suffix}</span></p></div>;
}

function ValidationRow({ issue, onSelect }: { issue: ValidationIssue; onSelect: (id: string) => void }) {
  const warning = issue.severity === "warning";
  return <button type="button" onClick={() => issue.nodeId && onSelect(issue.nodeId)} className={`flex w-full items-start gap-2 rounded-lg border px-2.5 py-2 text-left ${warning ? "border-amber-100 bg-amber-50/70" : "border-slate-100 bg-slate-50"} ${issue.nodeId ? "hover:border-slate-300" : "cursor-default"}`}>{warning ? <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" /> : <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-500" />}<span className="text-[10px] leading-relaxed text-slate-600">{issue.message}</span></button>;
}

const inputClass = "w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-800 outline-none focus:border-indigo-400";
...
function SetInspector({ node, patchSelected, poolUnit }: { node: StudioSetNode; patchSelected: (patcher: (node: StudioNode) => StudioNode) => void; poolUnit: PoolUnit }) {
  ...
  return (
    <>
      ...
      <div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><FieldLabel>Timing mode</FieldLabel><div className="grid grid-cols-2 gap-2">{(["send-off", "rest", "target-time", "open"] as const).map((mode) => <button type="button" key={mode} onClick={() => patch({ intervalMode: mode })} className={`rounded-lg border px-2 py-2 text-[10px] font-bold ${node.intervalMode === mode ? "border-indigo-300 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-white text-slate-500"}`}>{mode}</button>)}</div><div className="mt-3 grid grid-cols-2 gap-3"><div><FieldLabel>Send-off / interval</FieldLabel><input value={node.interval} onChange={(event) => patch({ interval: event.target.value })} className={`${inputClass} font-mono`} /></div><div><FieldLabel>Target time</FieldLabel><input value={node.targetTime} onChange={(event) => patch({ targetTime: event.target.value })} placeholder="0:30" className={`${inputClass} font-mono`} /></div>{node.intervalMode === "rest" && <div><FieldLabel>Rest seconds</FieldLabel><input type="number" min={0} value={node.restSeconds} onChange={(event) => patch({ restSeconds: Math.max(0, Number(event.target.value) || 0) })} className={inputClass} /></div>}</div></div>
      <div><div className="flex items-center justify-between"><FieldLabel>Intensity</FieldLabel><span className="text-xs font-bold text-slate-700">RPE {node.intensity}/10</span></div><input type="range" min={1} max={10} value={node.intensity} onChange={(event) => patch({ intensity: Number(event.target.value) })} className="w-full accent-indigo-600" /></div>
...
function InfoBox({ tone, children }: { tone: string; children: React.ReactNode }) {
  const styles: Record<string, string> = { amber: "border-amber-100 bg-amber-50 text-amber-800", sky: "border-sky-100 bg-sky-50 text-sky-800", violet: "border-violet-100 bg-violet-50 text-violet-800", orange: "border-orange-100 bg-orange-50 text-orange-800", indigo: "border-indigo-100 bg-indigo-50 text-indigo-800", pink: "border-pink-100 bg-pink-50 text-pink-800", yellow: "border-yellow-100 bg-yellow-50 text-yellow-800" };
