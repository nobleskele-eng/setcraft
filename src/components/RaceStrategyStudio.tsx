...
const INPUT = "w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 text-sm font-bold text-slate-950 outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100";
...
export default function RaceStrategyStudio() {
  ...
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-[26px] border border-slate-200 bg-slate-950 text-white">
      <div className="bg-[radial-gradient(circle_at_top_right,rgba(139,92,246,.32),transparent_38%)] p-6 md:p-8">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div><div className="flex items-center gap-2 text-cyan-300"><Sparkles className="h-4 w-4" /><span className="text-[10px] font-black uppercase tracking-[.2em]">Professional strategy workspace</span></div><h2 className="mt-2 text-3xl font-black">Plan. Compare. Brief.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Build an exact race plan, control every athlete factor independently, and ground the recommendation in official comparison swims.</p></div>
          <div className="grid grid-cols-3 gap-2 rounded-2xl border border-white/10 bg-white/[.06] p-2">{(["LCM", "SCM", "SCY"] as Course[]).map((item) => <button type="button" key={item} onClick={() => switchCourse(item)} className={`rounded-xl px-5 py-3 text-xs font-black ${course === item ? "bg-cyan-300 text-slate-950" : "text-slate-300 hover:bg-white/10"}`}>{item}</button>)}</div>
        </div>
      </div>
    </section>
    ...
    ] as Array<[StudioTab, React.ElementType, string]>).map(([id, Icon, label]) => <button type="button" key={id} onClick={() => setTab(id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-3 text-xs font-black ${tab === id ? "bg-slate-950 text-white" : "text-slate-500 hover:bg-slate-50"}`}><Icon className="h-4 w-4" />{label}</button>)}</nav>
    ...
    {tab === "plan" && <div className="grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
      ...
      <div className="space-y-6"><StudioCard eyebrow="Recommended architecture" title={selected?.name || "Race strategy"} action={<span className="rounded-full bg-violet-100 px-3 py-1.5 text-xs font-black text-violet-700">{selected?.fit || 0}% fit</span>}>
        <div className="grid gap-3 md:grid-cols-3">{ranked.map((item) => <button type="button" key={item.id} onClick={() => setStrategyId(item.id)} className={`rounded-2xl border p-4 text-left ${item.id === selected?.id ? "border-violet-400 bg-violet-50 ring-4 ring-violet-100" : "border-slate-200 hover:border-violet-200"}`}><div className="flex items-center justify-between gap-2"><p className="text-sm font-black text-slate-950">{item.name}</p><span className="font-mono text-xs font-black text-violet-700">{item.fit}%</span></div><p className="mt-2 text-xs leading-5 text-slate-500">{item.description}</p></button>)}</div>
        <RaceTable plan={plan} course={course} cue={selected?.name || "Hold shape"} />
        <div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={exportPdf} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-xs font-black text-white"><Download className="h-4 w-4" />Export strategy PDF</button><button type="button" onClick={() => downloadJson("setcraft-race-strategy-v13.json", payload)} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-xs font-black text-slate-700"><Layers3 className="h-4 w-4" />Export JSON</button></div>
      </StudioCard></div>
    </div>}
    ...
    {tab === "references" && <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]"><StudioCard eyebrow="Reference filter" title="Comparable field"><Field label="Performance level"><select className={INPUT} value={referenceLevel} onChange={(e) => setReferenceLevel(e.target.value as typeof referenceLevel)}><option>All</option>{LEVELS.map((item) => <option key={item}>{item}</option>)}</select></Field><div className="mt-5 space-y-2 text-xs text-slate-600"><p className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-500" />Official result files</p><p className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-blue-500" />Minor names anonymized</p><p className="flex items-center gap-2"><Database className="h-4 w-4 text-violet-500" />{OBSERVED_RACE_LIBRARY_MANIFEST.includedSwims.toLocaleString()} indexed races</p></div></StudioCard><StudioCard eyebrow="Nearest examples" title={`${event} ${course} · ${sex}`}><div className="grid gap-4 lg:grid-cols-2">{references.map((reference) => <ReferenceCard key={reference.id} reference={reference} />)}{!references.length && <p className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm font-bold text-slate-500">No reference matches this filter.</p>}</div></StudioCard></div>}

    {tab === "brief" && <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]"><StudioCard eyebrow="Coach Block AI" title="Evidence-locked execution brief" action={<button type="button" onClick={generateBrief} disabled={aiLoading} className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-3 text-xs font-black text-white disabled:opacity-60">{aiLoading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}{aiLoading ? "Drafting…" : "Generate brief"}</button>}>{aiError && <p className="mb-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-900">{aiError}. The verified offline brief is shown.</p>}<div className="whitespace-pre-wrap rounded-2xl bg-slate-950 p-5 text-sm leading-7 text-slate-200">{aiText || offlineBrief}</div></StudioCard><div className="space-y-4"><Insight icon={Gauge} label="Best fit" text={selected?.bestFor || "—"} tone="emerald" /><Insight icon={Trophy} label="Primary risk" text={selected?.risk || "—"} tone="rose" /><Insight icon={FlaskConical} label="Profile evidence" text={profileSummary(profile)} tone="blue" /><button type="button" onClick={exportPdf} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-4 text-sm font-black text-white"><Download className="h-4 w-4" />Export strategy PDF</button></div></div>}
  </div>;
}

function StudioCard({ eyebrow, title, action, children }: { eyebrow: string; title: string; action?: React.ReactNode; children: React.ReactNode }) { return <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm md:p-6"><div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-violet-600">{eyebrow}</p><h3 className="mt-1 text-xl font-black text-slate-950">{title}</h3></div>{action}</div>{children}</section>; }
...
function MiniMetric({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl bg-slate-100 p-4"><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 font-mono text-xl font-black text-slate-950">{value}</p></div>; }
function RaceTable({ plan, course, cue }: { plan: Array<{ point: number; split: number; cumulative: number }>; course: Course; cue: string }) { return <div className="mt-5 overflow-x-auto rounded-2xl border border-slate-200"><table className="w-full min-w-[620px] text-left text-sm"><thead className="bg-slate-950 text-white"><tr>{["Checkpoint", "Segment", "Cumulative", "Execution cue"].map((item) => <th key={item} className="px-4 py-3 text-[10px] font-black uppercase tracking-wide">{item}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{plan.map((item, index) => <tr key={item.point}><td className="px-4 py-3 font-black">{item.point} {courseUnit(course)}</td><td className="px-4 py-3 font-mono font-black text-violet-700">{formatTime(item.split)}</td><td className="px-4 py-3 font-mono">{formatTime(item.cumulative)}</td><td className="px-4 py-3 text-xs text-slate-500">{index === 0 ? "Clean commitment" : index === plan.length - 1 ? "Finish through wall" : cue}</td></tr>)}</tbody></table></div>; }
function ReferenceCard({ reference }: { reference: RaceReference }) { const aligned = alignReferenceToAnalysis(reference); return <article className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-slate-950">{reference.swimmer}</p><p className="mt-1 text-[11px] text-slate-500">{reference.meet} · {reference.round || "Result"}</p></div><div className="text-right"><p className="font-mono text-lg font-black text-violet-700">{formatTime(reference.total)}</p><p className="text-[10px] font-black uppercase text-slate-400">{reference.level}</p></div></div><div className="mt-3 flex flex-wrap gap-1.5">{referenceRows(aligned).map((row) => <span key={row.checkpoint} className={`rounded-lg px-2 py-1 font-mono text-[10px] font-bold ${row.provenance === "estimated" ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"}`}>{row.checkpoint}: {formatTime(row.cumulative)}</span>)}</div><p className="mt-3 text-[10px] font-bold text-slate-500">{reference.archetype} · green measured / amber projected</p></article>; }
function Insight({ icon: Icon, label, text, tone }: { icon: React.ElementType; label: string; text: string; tone: "emerald" | "rose" | "blue" }) { const colors = { emerald: "border-emerald-200 bg-emerald-50 text-emerald-950", rose: "border-rose-200 bg-rose-50 text-rose-950", blue: "border-blue-200 bg-blue-50 text-blue-950" }[tone]; return <div className={`rounded-2xl border p-5 ${colors}`}><Icon className="h-5 w-5" /><p className="mt-3 text-[10px] font-black uppercase tracking-wide">{label}</p><p className="mt-2 text-sm font-bold leading-6">{text}</p></div>; }
