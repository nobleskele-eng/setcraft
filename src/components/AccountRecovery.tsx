...
export default function AccountRecovery({ mode }: { mode: "forgot" | "reset" }) {
  ...
  return (
    <main className="sc-account-page sc-recovery-page">
      <section className="sc-account-visual" aria-label="SetCraft account security">
        <Link className="sc-account-back" href="/"><ArrowLeft /> Back to SetCraft</Link>
        <div className="sc-account-visual-content"><span className="sc-account-icon">{isForgot ? <Mail /> : <KeyRound />}</span><p className="sc-account-overline">SetCraft account security</p><h1>{isForgot ? "Get back to the work behind the work." : "Choose a stronger way back in."}</h1><p>{isForgot ? "Request a short-lived, single-use reset link without revealing whether an email is registered." : "A successful reset changes the password and signs out every previous SetCraft session."}</p></div>
