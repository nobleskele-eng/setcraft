    ...
    <div className="mx-auto w-full max-w-[1900px] space-y-8 pb-20">
      <header className="relative overflow-hidden rounded-[32px] border border-hairline bg-surface p-8 text-white shadow-md shadow-surface/10 lg:p-10">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-7">
          <div>
            <div className="flex items-center gap-2 text-ink-muted"><FolderOpen className="h-6 w-6" /><span className="text-xs font-bold uppercase tracking-[0.2em]">Season workspace</span></div>
            <h1 className="mt-4 font-display text-4xl font-bold tracking-tight lg:text-5xl">Project Hub</h1>
            <p className="mt-3 max-w-3xl text-base leading-relaxed text-ink-muted lg:text-lg">Create practices, build season folders, and reopen every workout as an editable visual program. Your saved work stays organized by phase, squad or meet cycle.</p>
          </div>
        ...
        <div className="relative mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[{ label: "Saved projects", value: projects.length.toString(), icon: Layers3 }, { label: "Season folders", value: folders.length.toString(), icon: Folder }, { label: "Programmed volume", value: `${totalDistance.toLocaleString()} m/yd`, icon: Waves }, { label: "Recently edited", value: recentProjects[0]?.name || "No project yet", icon: Clock3 }].map((item) => { const Icon = item.icon; return <div key={item.label} className="rounded-2xl border border-white/10 bg-white/[0.065] p-4 backdrop-blur"><div className="flex items-center gap-2 text-ink-muted-on-canvas"><Icon className="h-4 w-4" /><span className="text-[10px] font-bold uppercase tracking-wider">{item.label}</span></div><p className="mt-2 truncate text-lg font-bold text-white">{item.value}</p></div>; })}
        </div>
      </header>
