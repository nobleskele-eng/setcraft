...
export const PALETTE_PRESETS: PalettePreset[] = [
  ...
  preset("dive-15-quality", "Speed", "Dive to 15 quality", "Full-recovery start and breakout repetitions", () => createSetNode("sprint", { label: "Dive to 15 quality", variant: "Start speed", reps: 8, distance: 15, interval: "1:30", startMethod: "Dive", intensity: 10, notes: "Record 15 m time; walk back recovery" })),
  preset("turn-in-out-15", "Skills", "Turn in / out 15", "Time the final 7.5 m in and first 7.5 m out", () => createSetNode("drill", { label: "Turn in / out 15", variant: "Turn speed", reps: 8, distance: 15, interval: "0:50", intensity: 8, skillFocus: "Fast approach, legal turn, tight streamline", notes: "Start 7.5 m from wall and finish 7.5 m out" })),
  preset("breakout-25-quality", "Underwater", "Breakout 25 quality", "Underwater speed into clean first strokes", () => createSetNode("underwater", { label: "Breakout 25 quality", variant: "Underwater to swim", reps: 8, distance: 25, interval: "0:55", equipment: ["Fins"], intensity: 8, notes: "Underwater to coach mark, then accelerate into stroke" })),
  preset("relay-exchange-skill", "Skills", "Relay exchange skill", "Reaction, timing and legal takeover practice", () => ({ id: uid("section"), kind: "section", title: "Relay exchange practice", purpose: "Develop safe, repeatable takeover timing", pointsOfPerformance: "Watch incoming swimmer; stable stance; legal contact; record reaction", children: [createSetNode("sprint", { label: "Relay takeover 25", reps: 8, distance: 25, interval: "1:15", startMethod: "Dive", intensity: 9, notes: "Coach verifies legal contact before feet leave" })], locked: false })),
  preset("easy-fast-50s", "Speed", "Easy / fast 50s", "Alternate relaxed swimming and high-quality speed", () => ({ id: uid("repeat"), kind: "repeat", label: "Easy / fast pair", rounds: 6, children: [createSetNode("recovery", { label: "Easy 50", reps: 1, distance: 50, interval: "1:00" }), createSetNode("sprint", { label: "Fast 50", reps: 1, distance: 50, interval: "1:30", intensity: 9 })], locked: false })),
  preset("ascending-ladder", "Aerobic", "25-50-75-100 ladder", "Ascending aerobic ladder with consistent pace", () => ({ id: uid("section"), kind: "section", title: "Ascending ladder", purpose: "Hold pace as repetition distance grows", pointsOfPerformance: "Even effort; clean turns; maintain stroke length", children: [25,50,75,100].map((distance) => createSetNode("aerobic", { label: `${distance} aerobic`, reps: 2, distance, interval: formatSeconds(distance / 100 * 95 + 15) })), locked: false })),
  preset("descending-ladder-quality", "Threshold", "100-75-50-25 descend", "Descending distance with increasing speed", () => ({ id: uid("section"), kind: "section", title: "Descending quality ladder", purpose: "Increase velocity as distance falls", pointsOfPerformance: "Controlled 100; faster each step; perfect final 25", children: [100,75,50,25].map((distance, index) => createSetNode(index < 2 ? "threshold" : "sprint", { label: `${distance} build speed`, reps: 2, distance, interval: formatSeconds(distance / 100 * 85 + 25), intensity: 7 + index })), locked: false })),
  preset("three-two-one-threshold", "Threshold", "3-2-1 threshold", "Three 100s, two 150s and one 300 at controlled threshold", () => ({ id: uid("section"), kind: "section", title: "3-2-1 threshold", purpose: "Accumulate threshold work through varied repetition lengths", pointsOfPerformance: "Same pace per 100; controlled rest; no late fade", children: [createSetNode("threshold", { label: "Threshold 100s", reps: 3, distance: 100, interval: "1:30" }), createSetNode("threshold", { label: "Threshold 150s", reps: 2, distance: 150, interval: "2:15" }), createSetNode("threshold", { label: "Threshold 300", reps: 1, distance: 300, interval: "4:30" })], locked: false })),
  preset("broken-100", "Race pace", "Broken 100", "Four 25s with coach-selected rest and cumulative time", () => createSetNode("race-pace", { label: "Broken 100", variant: "4 × 25 broken", reps: 4, distance: 25, intervalMode: "rest", restSeconds: 10, interval: "0:25", targetTime: "0:15", intensity: 9, notes: "Add swim times for broken total" })),
  preset("broken-200", "Race pace", "Broken 200", "Four 50s with short fixed rest", () => createSetNode("race-pace", { label: "Broken 200", variant: "4 × 50 broken", reps: 4, distance: 50, intervalMode: "rest", restSeconds: 10, interval: "0:45", targetTime: "0:32", intensity: 9, notes: "Record each split and cumulative swim time" })),
  preset("broken-400", "Race pace", "Broken 400", "Four 100s with controlled rest", () => createSetNode("race-pace", { label: "Broken 400", variant: "4 × 100 broken", reps: 4, distance: 100, intervalMode: "rest", restSeconds: 15, interval: "1:30", targetTime: "1:10", intensity: 9, notes: "Preserve intended race shape" })),
  preset("best-average-50s", "Speed", "Best-average 50s", "Repeatable speed benchmark with full recording", () => createSetNode("test-set", { label: "Best-average 50s", variant: "Best average", reps: 8, distance: 50, interval: "2:00", intensity: 10, notes: "Record all times, average and range; stop if technique fails" })),
  preset("pace-clock-50s", "Aerobic", "Pace-clock 50s", "Clock-reading and pacing consistency set", () => createSetNode("aerobic", { label: "Pace-clock 50s", variant: "Clock skill", reps: 12, distance: 50, interval: "0:55", intensity: 6, notes: "Call finish time and calculate rest before next send-off" })),
  preset("kick-swim-transfer", "Kick", "Kick to swim transfer", "Move from isolated kick into full-stroke speed", () => ({ id: uid("repeat"), kind: "repeat", label: "Kick-to-swim transfer", rounds: 4, children: [createSetNode("kick", { label: "Fast kick", reps: 2, distance: 50, interval: "1:00", intensity: 8 }), createSetNode("sprint", { label: "Transfer swim", reps: 2, distance: 25, interval: "0:45", intensity: 9 })], locked: false })),
  preset("pull-swim-transfer", "Pull", "Pull to swim transfer", "Transfer catch pressure into normal swimming", () => ({ id: uid("repeat"), kind: "repeat", label: "Pull-to-swim transfer", rounds: 4, children: [createSetNode("pull", { label: "Strong pull", reps: 2, distance: 100, interval: "1:35", equipment: ["Pull buoy", "Paddles"], intensity: 7 }), createSetNode("threshold", { label: "Swim with same catch", reps: 1, distance: 100, interval: "1:25", equipment: [], intensity: 8 })], locked: false })),
  preset("vertical-kick-circuit", "Kick", "Vertical-kick circuit", "Short vertical kick bouts paired with swimming", () => ({ id: uid("repeat"), kind: "repeat", label: "Vertical-kick circuit", rounds: 6, children: [createSetNode("kick", { label: "Vertical kick", reps: 1, distance: 0, interval: "0:30", intervalMode: "rest", targetTime: "0:20", restSeconds: 10, equipment: [], intensity: 8, notes: "20 seconds vertical; hands streamlined only if appropriate" }), createSetNode("sprint", { label: "Fast swim", reps: 1, distance: 25, interval: "0:45", intensity: 9 })], locked: false })),
  preset("im-transition-50s", "Skills", "IM transition 50s", "Practice the wall between consecutive IM strokes", () => createSetNode("drill", { label: "IM transition 50s", variant: "Transition skill", reps: 12, distance: 50, stroke: "IM Order", interval: "1:10", intensity: 6, notes: "25 into the transition + 25 out; legal finish and turn" })),
  preset("event-finish-25s", "Speed", "Event finish 25s", "Closing-speed repetitions from a fatigued start", () => createSetNode("sprint", { label: "Event finish 25s", variant: "Finish speed", reps: 8, distance: 25, interval: "0:50", intensity: 10, startMethod: "Push", notes: "Start with 5 seconds fast feet at wall, then sprint through finish" })),
  preset("meet-warmup-full", "Warm-up", "Meet warm-up template", "General warm-up, skill, pace and start readiness", () => ({ id: uid("section"), kind: "section", title: "Meet warm-up", purpose: "Raise readiness without creating unnecessary fatigue", pointsOfPerformance: "Progress gradually; rehearse race skills; finish feeling sharp", children: [createSetNode("warm-up", { label: "Easy progressive swim", reps: 1, distance: 500, interval: "10:00" }), createSetNode("drill", { label: "Event drill / swim", reps: 4, distance: 50, interval: "1:00" }), createSetNode("race-pace", { label: "Race-pace 25s", reps: 4, distance: 25, interval: "0:50", intensity: 8 }), createSetNode("sprint", { label: "Start to 15", reps: 2, distance: 15, interval: "2:00", startMethod: "Dive", intensity: 9 }), createSetNode("recovery", { label: "Easy reset", reps: 1, distance: 200, interval: "4:00" })], locked: false })),
  preset("css-200-400-test", "Threshold", "200 / 400 critical-speed test", "Two maximal swims used for a critical-speed reference", () => ({ id: uid("section"), kind: "section", title: "Critical-speed test", purpose: "Collect consistent 200 and 400 results for the calculator", pointsOfPerformance: "Same pool and stroke; accurate timing; full coach-selected recovery", children: [createSetNode("test-set", { label: "Timed 400", reps: 1, distance: 400, interval: "10:00", intervalMode: "target-time", targetTime: "5:00", intensity: 10, notes: "Record all 100 splits" }), createSetNode("recovery", { label: "Full recovery", reps: 1, distance: 400, interval: "12:00", notes: "Coach may extend recovery" }), createSetNode("test-set", { label: "Timed 200", reps: 1, distance: 200, interval: "6:00", intervalMode: "target-time", targetTime: "2:20", intensity: 10, notes: "Record 50 splits" })], locked: false })),
...
function nodeStats(node: StudioNode): ReturnType<typeof emptyStats> {
  ...
  if (node.kind === "set") {
    ...
    const intervalSeconds = parseInterval(node.interval);
    const targetSeconds = parseInterval(node.targetTime);
    const intervalSec = node.intervalMode === "rest"
      ? (targetSeconds > 0 ? targetSeconds : intervalSeconds) + Math.max(0, node.restSeconds)
      : node.intervalMode === "target-time"
        ? (targetSeconds > 0 ? targetSeconds : intervalSeconds)
        : node.intervalMode === "open"
          ? Math.max(intervalSeconds, targetSeconds)
          : intervalSeconds;
...
export function validateWorkout(nodes: StudioNode[], targetMinutes: number, poolLength: PoolLength = 25): ValidationIssue[] {
  ...
  flatSets.forEach((set) => {
    ...
    if (set.intervalMode === "rest" && parseInterval(set.targetTime) <= 0 && parseInterval(set.interval) <= 0) issues.push({ id: `rest-time-${set.id}`, nodeId: set.id, severity: "warning", message: `${set.label}: fixed-rest mode needs an expected swim time or interval so duration can be estimated.` });
    if (set.intervalMode === "send-off" && parseInterval(set.targetTime) > 0 && parseInterval(set.interval) > 0 && parseInterval(set.targetTime) >= parseInterval(set.interval)) issues.push({ id: `negative-rest-${set.id}`, nodeId: set.id, severity: "warning", message: `${set.label}: target time is not faster than the send-off, so the set creates zero or negative rest.` });
    if (set.intensity >= 9 && set.reps * set.distance >= 1000) issues.push({ id: `quality-volume-${set.id}`, nodeId: set.id, severity: "info", message: `${set.label}: ${set.reps * set.distance} of RPE 9–10 work is programmed in one block. Confirm that quality and recovery are realistic.` });
  });
  ...
  allNodes.forEach((node) => {
    ...
    if (node.kind === "section" && node.children.length === 0) issues.push({ id: `empty-${node.id}`, nodeId: node.id, severity: "info", message: `${node.title}: this section is empty.` });
    if (node.kind === "condition" && node.children.length === 0) issues.push({ id: `empty-${node.id}`, nodeId: node.id, severity: "info", message: `${node.label}: add the set or sequence controlled by this condition.` });
    ...
    if (node.kind === "time-cap" && node.children.length === 0) issues.push({ id: `empty-${node.id}`, nodeId: node.id, severity: "info", message: `${node.label}: add work inside the time cap.` });
    ...
    if (node.kind === "lane" && node.children.length === 0) issues.push({ id: `empty-${node.id}`, nodeId: node.id, severity: "info", message: `${node.label}: add the shared set that each lane will adapt.` });
  });
