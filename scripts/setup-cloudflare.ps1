...
function Get-D1Databases {
  ...
  # Support both Wrangler's direct array response and wrapped API responses.
  if ($null -ne $Parsed.PSObject.Properties["result"]) {
    $Parsed = $Parsed.result
  }

  foreach ($Database in @($Parsed)) {
    if (
      $null -ne $Database -and
      $null -ne $Database.PSObject.Properties["name"]
    ) {
